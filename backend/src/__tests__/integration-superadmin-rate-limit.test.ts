...
describe('SUPERADMIN Rate Limit Integration Test', () => {
  ...
  test(
    'SUPERADMIN should bypass rate limit completely',
    async () => {
      if (!superadminToken) {
      ...
      console.log(
        `SUPERADMIN - Total: ${responses.length}, Successful: ${successful.length}, Rate Limited: ${rateLimited.length}`
      );
  ...
  test(
    'Regular USER should hit rate limit',
    async () => {
      if (!regularUserToken) {
      ...
      console.log(
        `USER - Total: ${responses.length}, Successful: ${successful.length}, Rate Limited: ${rateLimited.length}`
      );
  ...
  test(
    'Verify rate limit log messages for SUPERADMIN',
    async () => {
      if (!superadminToken) {
        console.warn('⚠️ Skipping test - no SUPERADMIN token available');
        return;
