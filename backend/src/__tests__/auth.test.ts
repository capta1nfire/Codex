...
describe('Authentication Tests', () => {
  ...
  beforeAll(async () => {
    ...
    app.post('/api/auth/admin', authenticateJwt, requireRole(UserRole.ADMIN), (req: Request, res: Response) => {
      res.status(200).json({ message: 'Admin access granted' });
    });
    
    app.post('/api/auth/premium', authenticateJwt, requireRole(UserRole.PREMIUM), (req: Request, res: Response) => {
