...
export const validateBody =
  (schema: ZodSchema<any>) => async (req: Request, _res: Response, next: NextFunction) => {
    try {
      ...
      if (error instanceof ZodError) {
        ...
        return next(
          new AppError('Error de validación', 400, ErrorCode.VALIDATION_ERROR, { issues })
        );
      }
      next(error);
    }
  ...
  (schema: ZodSchema<any>) => async (req: Request, _res: Response, next: NextFunction) => {
    try {
      ...
      if (error instanceof ZodError) {
        ...
        return next(
          new AppError('Error de validación en parámetros', 400, ErrorCode.VALIDATION_ERROR, {
            issues,
          })
        );
      }
