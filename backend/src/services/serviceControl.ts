...
export async function startDatabaseService(): Promise<ServiceResult> {
  try {
    ...
    const { stdout: containerList } = await execAsync('docker ps -a --format "{{.Names}}" | grep codex_postgres || true');
    ...
    if (!containerList.trim()) {
      ...
      try {
        ...
        return {
          success: false,
          message: 'Database container not found and failed to create. Please run docker-compose up manually.'
        };
      }
    }
    
    // Check if container is already running
    const { stdout: runningCheck } = await execAsync('docker ps --format "{{.Names}}" | grep codex_postgres || true');
    ...
    for (let i = 0; i < 15; i++) { // Wait up to 15 seconds
      try {
        await execAsync('docker exec codex_postgres pg_isready -U codex_user -d codex_db', { timeout: 3000 });
        dbReady = true;
        break;
      } catch {
...
export async function stopDatabaseService(): Promise<ServiceResult> {
  try {
    ...
    const { stdout: verifyStop } = await execAsync('docker ps --format "{{.Names}}" | grep codex_postgres || true');
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      childProcess.stdout?.on('data', (data) => {
        ...
        if (data.toString().includes('listening on') || data.toString().includes('Server running')) {
  ...
  try {
    ...
    if (isDevelopment) {
      ...
      try {
        ...
        const { stdout: systemdCheck } = await execAsync('systemctl is-active codex-backend 2>/dev/null || true');
        
        if (systemdCheck.trim() === 'active') {
          logger.info('🎯 Systemd service detected, attempting systemd restart...');
          const { stdout: systemdRestart } = await execAsync('sudo systemctl restart codex-backend');
        ...
        return {
          ...
          details: { 
            ...
            instructions: 'No PM2 or systemd detected. Please restart manually or configure a process manager.'
          }
        };
...
export async function getServiceStatus(serviceName: string): Promise<ServiceResult> {
  try {
    ...
    switch (serviceName.toLowerCase()) {
      case 'database':
        const { stdout: dbCheck } = await execAsync('docker ps --format "{{.Names}}:{{.Status}}" | grep codex_postgres || true');
        if (dbCheck.trim()) {
      ...
      case 'rust_generator':
        ...
        const isHealthy = portOccupied ? await waitForService('http://localhost:3002/health', 1) : false;
