...
const generateSmartQRUseCase = new GenerateSmartQRUseCase(templateService);
    ...
    try {
      ...
      const result = await generateSmartQRUseCase.execute({
        ...
        options: {
          ...
          returnFullTemplate: user.role === 'admin'
        }
      });
      ...
      if (!result.success) {
        const statusCode = 
          result.error?.code === 'AUTHENTICATION_REQUIRED' ? 401 :
          result.error?.code === 'LIMIT_REACHED' ? 429 :
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const limitCheck = await limitService.checkLimit(user.id, isPremium);
