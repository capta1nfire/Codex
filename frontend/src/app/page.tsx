...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    const options: Record<string, unknown> = { scale };
    // if (type === 'qrcode' && errorLevel) { // Comment out usage of errorLevel
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 xl:gap-10 print:block">
          ...
          <div className="space-y-6 print:hidden">
            ...
            <div className="bg-white p-6 border rounded-lg shadow-md">
              ...
              <div className="mb-4">
                ...
                />
                {/* Ayuda contextual */}
                <div className="mt-1.5 text-xs text-gray-600 min-h-[1.2em]">
                  {' '}
                  {/* Placeholder para altura */}
                  {type === 'ean13' && (
                    <span>Exactamente 12 dígitos numéricos (checksum automático)</span>
                  )}
                  ...
                  <div>
                    ...
                    <Disclosure.Panel className="p-4 border-t">
                      <div className="grid grid-cols-1 gap-4">
                        ...
                        <div>
                          {/* <Label htmlFor="scaleSlider">Escala (tamaño): {scale}x</Label> */}
                          <Label htmlFor="scaleSlider" className="text-sm text-gray-700 mb-2">
                            Escala (tamaño): {scale}x
                          </Label>
                          <input
                            type="range"
                            id="scaleRange"
          ...
          <div className="flex flex-col">
            ...
            <div className="bg-white p-6 border rounded-lg shadow-md flex flex-col items-center justify-center h-full min-h-[350px] print:border-none print:shadow-none print:p-0 print:bg-transparent">
              <h2 className="text-xl font-semibold mb-4 text-gray-800 print:hidden">
                Previsualización
              </h2>
              <div className="flex-grow flex items-center justify-center w-full print:block print:w-auto print:h-auto print:flex-grow-0">
                {isLoading ? (
                  <p className="text-blue-600 animate-pulse print:hidden">Generando código...</p>
                ) : error ? (
                  <p className="text-red-600 print:hidden">
                    Error al generar. Revisa la configuración.
                  </p>
                ) : svgContent ? (
                  // Contenedor del SVG - Ahora más adaptable
                  <div
                    dangerouslySetInnerHTML={{ __html: svgContent }}
                    className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl print:max-w-full print:w-full"
                    role="img"
