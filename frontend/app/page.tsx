    ...
    <main className="flex min-h-screen flex-col items-center p-4 sm:p-8 print:p-0 bg-gray-50 text-gray-900">
      ...
      <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 gap-6 print:block">
        ...
        <div className="space-y-6 print:hidden">
          ...
          {/* Sección de Personalización (usando useState en lugar de Disclosure) */}
          <div className="bg-white border rounded-lg shadow-md overflow-hidden">
            <button
              onClick={() => setIsCustomOptionsOpen(!isCustomOptionsOpen)}
              className="flex justify-between w-full px-4 py-3 text-sm font-medium text-left text-blue-900 bg-blue-100 hover:bg-blue-200 focus:outline-none focus-visible:ring focus-visible:ring-blue-500 focus-visible:ring-opacity-75"
            >
              <span>Opciones de Personalización</span>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" 
                className={`w-5 h-5 transform transition-transform ${isCustomOptionsOpen ? 'rotate-180' : ''}`}>
                <path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
              </svg>
            </button>
            
            {isCustomOptionsOpen && (
              <div className="px-4 pt-4 pb-4 bg-white transition-all duration-300 ease-in-out text-gray-800">
                <div className="space-y-4">
                  <fieldset className="border p-4 rounded border-gray-300">
                    <legend className="font-semibold text-sm px-1 text-gray-700">Apariencia</legend>
                  ...
                  {showErrorCorrection && (
                    <fieldset className="border p-4 rounded border-gray-300">
                      <legend className="font-semibold text-sm px-1 text-gray-700">Opciones Avanzadas (QR)</legend>
                      <div className="flex flex-col gap-4 pt-2">
                        <div>
      ...
      {/* Copyright y versión en el pie de página */}
      <footer className="w-full mt-12 text-center text-sm text-gray-500 print:hidden">
        <p>CODEX - Plataforma de Generación de Códigos de Barras y QR</p>
        <p className="text-xs mt-1">Versión 1.0.0 - Desarrollado con Next.js, Node.js y Rust</p>
      </footer>
      
      {/* Modal de Estado */}
      {isHealthModalOpen && <HealthModal isOpen={isHealthModalOpen} onClose={() => setIsHealthModalOpen(false)} healthData={healthInfo} />}
    </main>
  );
}