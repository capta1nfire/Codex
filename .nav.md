...
3. **Use PM2**: Never use `npm run dev` directly
4. **Check types**: Run TypeScript checks before committing
5. **Test first**: Write tests for new features
6. **Document**: Update EXISTING docs only (CHANGELOG.md priority)
7. **API Version**: QR → v2 (performance), Others → v1 (stable)
8. **Every 30 min**: Run `./scripts/validate-focus.sh`
